...
const buildBrowsePixel = (filterWord: string, offset: number) => {
  ...
  if (trimmed) {
    return `MyEngineProject ( metaKeys = ${MCP_META_KEYS} , metaFilters = ${MCP_META_FILTERS} , type = [ "PROJECT" , "STORAGE" , "DATABASE" , "FUNCTION" , "MODEL" ] , filterWord = "${trimmed}" , limit = [ ${MCP_PAGE_SIZE} ] , offset = [ ${offset} ] ) ;`;
  }
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(callGetUserMcps.fulfilled, (state, action) => {
      ...
      if (defaultMatches.length > 0) {
        state.pinnedItems = mergeUniqueMcps(state.pinnedItems, defaultMatches);
