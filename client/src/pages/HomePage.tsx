...
export const HomePage = () => {
	...
	const isCreateDisabled =
		trimmedProjectName.length === 0 || isCreatingProject;

	// Derive base URL at runtime: current page origin + MODULE from injected semoss-env.
	let iframeSrc = "";
	if (projectId) {
		const runtimeModule =
			Env.MODULE || import.meta.env.MODULE || "/Monolith";
					...
					<TooltipProvider delayDuration={100}>
						{roomId && (
							<Tooltip>
								...
								>
									<p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-0.5">Room ID</p>
						...
						{projectId && (
							<Tooltip>
								...
								<TooltipContent
									...
									className="max-w-xs break-all rounded-lg bg-popover px-3 py-2 text-xs text-popover-foreground shadow-lg border border-slate-200/50 dark:border-white/10"
								>
									<p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-0.5">Project ID</p>
					...
					<div className="flex h-full w-full flex-col items-center justify-center gap-2 px-6 text-center">
						...
						{showCreateProjectMessage ? (
							<p className="text-xs text-muted-foreground">
								Use the Create button to spin up your first
								workspace.
							</p>
						) : null}
						{showLoadingMessage ? (
